...
import { type Square, Chess } from '@/lib/chess/chess';
...
export function VisionView() {
  ...
  // Save vision session once when results are shown
  useEffect(() => {
    ...
    fetch('/api/training/vision', {
      ...
      body: JSON.stringify({
        ...
        accuracy: attempts.length > 0 ? score / attempts.length : 0,
        avgResponseTimeMs: averageResponseTime
      })
    }).catch(() => {});
  }, [gameStatus, session?.user?.id, trainingMode, colorMode, timeLimit, score, attempts, averageResponseTime]);
