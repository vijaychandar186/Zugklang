...
describe('challenge flow integration', () => {
  ...
  test('create challenge, block self-join, then join from second player', async () => {
    ...
    const selfJoinError = findMessage<{
      type: 'error';
      message: string;
    }>(creator.sent, 'error');
    expect(selfJoinError?.message).toContain("can't join your own");
    await handleJoinChallenge(asBunWs(joiner), { challengeId: created!.challengeId });
    expect(challenges.has(created!.challengeId)).toBe(false);
    expect(rooms.size).toBe(1);
    const creatorMatched = findMessage<{
      type: 'matched';
      color: string;
    }>(creator.sent, 'matched');
    const joinerMatched = findMessage<{
      type: 'matched';
      color: string;
    }>(joiner.sent, 'matched');
