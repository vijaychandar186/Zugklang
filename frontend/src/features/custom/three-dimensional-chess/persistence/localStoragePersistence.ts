...
export class LocalStoragePersistence implements GamePersistence {
  async listSaves(): Promise<SaveListEntry[]> {
    ...
    const hasKingPawnAndBishops = index.some(
      (e) => e.name === 'King, Pawn and Bishops'
    );
    if (!hasKingPawnAndBishops) {
      const kingPawnAndBishopsPieces = [
        {
          id: 'white-king-0',
          type: 'king',
          color: 'white',
      ...
      const doc: PersistedGameState = {
        ...
        updatedAt: now,
        id,
        name: 'King, Pawn and Bishops',
        payload: {
          pieces: kingPawnAndBishopsPieces,
          currentTurn: 'white',
          isCheck: false,
          isCheckmate: false,
          isStalemate: false,
    ...
    const hasKingsPawnsRooks = index.some(
      (e) => e.name === 'Kings, pawns and rooks'
    );
  ...
  async saveGame(
    entry: Omit<PersistedGameState, 'createdAt' | 'updatedAt' | 'id'> &
      Partial<Pick<PersistedGameState, 'id'>>
    ...
    const doc: PersistedGameState = {
      ...
      id,
      createdAt: entry.id
        ? ((entry['createdAt' as keyof typeof entry] as string) ?? now)
        : now,
      updatedAt: now,
  ...
  async loadGame(id: string): Promise<PersistedGameState | null> {
    ...
    if (parsed.data.version !== SCHEMA_VERSION) {
      throw new Error(
        `Unsupported save version ${parsed.data.version}, expected ${SCHEMA_VERSION}`
      );
    }
    return parsed.data;
  }
