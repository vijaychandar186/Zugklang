...
def build_dense_layer_data(insights_df: pd.DataFrame, db: Prisma) -> pd.DataFrame:
    df = (
        insights_df[["user", "tc", "days"]]
        .drop_duplicates()
        .merge(
            insights_df[
                (insights_df["insight"] == "acpl_result") & (insights_df["bin"] == 1)
            ][["user", "tc", "days", "nb", "value"]].rename(
                columns={"nb": "nb1", "value": "value1"}
            ),
            how="left",
        )
        .merge(
            insights_df[
                (insights_df["insight"] == "acpl_result") & (insights_df["bin"] == 3)
            ][["user", "tc", "days", "nb", "value"]].rename(
                columns={"nb": "nb3", "value": "value3"}
            ),
            how="left",
        )
        .merge(
            ...
            .groupby(["user", "tc", "days"], as_index=False)
            .sum()
            .rename(columns={"nb": "eval_ss"}),
            how="left",
        )
        .merge(
            insights_df[
                (insights_df["insight"] == "movetime_result")
                & (insights_df["bin"] == 1)
            ][["user", "tc", "days", "nb"]]
            .groupby(["user", "tc", "days"], as_index=False)
            .sum()
            .rename(columns={"nb": "all_nb1"}),
            how="left",
        )
        .merge(
            insights_df[
    ...
    df["num_map"] = df["bin"].map(
        dict(zip(dense_fields, range(len(dense_fields)), strict=False))
    )
...
def _get_user_date_window(
    ...
    try:
        ...
        return (
            None
            if is_marked
            else {tc: player_date_index[username][tc] for tc in time_controls}
        )
...
def partition_users(
    db: Prisma,
    is_live: bool = False,
    live_user_list: list[str] | None = None,
...
def build_eligible_players(
    ...
    eligible: dict = {
        tc: {days: [] for days in date_bucket_config} for tc in time_controls
    }
    for tc in legit_first_df["tc"].unique():
        for days in legit_first_df["days"].unique():
            eligible[tc][days] = list(
                cheat_first_df[
                    (cheat_first_df["tc"] == tc) & (cheat_first_df["days"] == days)
                ]["user"].unique()
            ) + list(
                legit_first_df[
                    (legit_first_df["tc"] == tc) & (legit_first_df["days"] == days)
                ]["user"].unique()
            )
    return eligible
...
def build_user_eligibility_map(
    ...
    result = {
        user: [
            ...
            if user in eligible_players[tc][days]
        ]
...
def _aggregate_first_insight(
    ...
    df = pd.DataFrame(
        rows, columns=["insight", "user", "tc", "days", "bin", "nb", "value"]
    )
    if not is_live:
...
def _run_game_metric_by_date(
    ...
    for idx, username in enumerate(user_eligibility.keys()):
        ...
        for tc, days in user_eligibility[username]:
            ...
            n_buckets = date_bucket_config[days]
            for i, row in enumerate(
                query_fn(db, username, tc, start, date_window[tc], max_games, n_buckets)
            ):
                rows.append([metric, username, tc, days, i, row["nb"], row["v"]])
...
def _run_metric_by_dimension(
    ...
    for idx, username in enumerate(user_eligibility.keys()):
        ...
        for tc, days in user_eligibility[username]:
            start = date_window[tc] - timedelta(days=days)
            for row in query_fn(
                db, username, tc, start, date_window[tc], max_games, max_moves
            ):
                rows.append(
                    [metric, username, tc, days, row["_id"], row["nb"], row["v"]]
                )
...
def _run_movetime_first_three_moves(
    ...
    for idx, username in enumerate(user_eligibility.keys()):
        ...
        for tc, days in user_eligibility[username]:
            start = date_window[tc] - timedelta(days=days)
            for game in first_three_move_times(
                db, username, tc, start, date_window[tc]
            ):
                rows.append(
                    [username, tc, days, game["id"], game["m1"], game["m2"], game["m3"]]
                )
    ...
    col_map = {
        col: c
        for c, col in enumerate(agg.columns)
        if col not in ["user", "tc", "days", "nb"]
    }
    agg = agg.melt(
        id_vars=["user", "tc", "days", "nb"], var_name="bin", value_name="value"
    )
...
def generate_all_insights(
    ...
    player_date_index: dict | None = None,
    max_games: int = 1000,
    max_moves: int = 20000,
    ...
    user_eligibility = build_user_eligibility_map(
        ...
        eligible_players,
        user_list,
        overwrite=overwrite_eligibility,
        is_marked=bool(is_marked),
        is_live=is_live,
        use_eval=use_eval,
    )
    output_dir = f"/tmp/insights_df_chunks/use_eval_{use_eval}/"
    ...
    for query_fn, metric in move_date_queries:
        chunk_list.append(
            _run_move_metric_by_date(
                query_fn=query_fn,
                metric=metric,
                max_games=max_games,
                max_moves=max_moves,
                date_bucket_config=date_bucket_config,
                **shared,
            )
        )
    for query_fn, metric in metric_dim_queries:
        chunk_list.append(
            _run_metric_by_dimension(
                query_fn=query_fn,
                metric=metric,
                max_games=max_games,
                max_moves=max_moves,
    ...
    for query_fn, metric in game_dim_queries:
        chunk_list.append(
            _run_game_metric_by_dimension(
                query_fn=query_fn, metric=metric, max_games=max_games, **shared
            )
        )

    chunk_list.append(
        _run_movetime_first_three_moves(
            db=db,
            user_eligibility=user_eligibility,
            time_controls=time_controls,
            is_marked=is_marked,
...
def build_composite_insight(
    df: pd.DataFrame, metric_a: str, metric_b: str
) -> pd.DataFrame:
    merged = (
        df[df["insight"] == f"{metric_a}_date"][
            ["user", "tc", "days", "bin", "nb", "value"]
        ]
        .rename(columns={"nb": f"nb_{metric_a}", "value": f"value_{metric_a}"})
        .merge(
            df[df["insight"] == f"{metric_b}_date"][
                ["user", "tc", "days", "bin", "nb", "value"]
...
def _get_users_missing_dimension(
    df: pd.DataFrame, user_list: list[str], dimension: str
) -> set:
    subset = df[
        (df["user"].isin(user_list)) & (df["insight"].str.contains(f"_{dimension}"))
    ]
...
def impute_missing_insight_values(
    df: pd.DataFrame, is_live: bool, use_eval: int
) -> pd.DataFrame:
    ...
    for dim in {insight.split("_")[1] for insight in df["insight"].unique()}:
        ...
        sub = (
            user_keys.merge(bin_keys[bin_keys["insight"].str.contains(f"_{dim}")])
            .merge(sub, how="left")
            .drop("_dummy", axis=1)
        )
...
def _compute_quantile_bounds(
    ...
    lower = (
        train_df.groupby(["insight", "tc", "days"], as_index=False)[["nb", "value"]]
        .quantile(lower_q)
...
def clip_and_scale_features(
    ...
    df["value"] = np.where(
        df["insight"] == "ratinggain_date", df["value"] + 10000, df["value"]
    )
    df["nb"] = np.log(df["nb"])
    ...
    df["value"] = (
        2 * (df["value"] - df["value_min"]) / (df["value_max"] - df["value_min"]) - 1
    )
