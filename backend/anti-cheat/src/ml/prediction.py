...
from ..db import db_retry, load_pickle, save_pickle
...
def _predict_single_config(
    ...
    explainer: shap.DeepExplainer | None,
    model_dir: str,
) -> pd.DataFrame:
...
def run_predictions(
    ...
    if data_dict is None:
        output_path = os.path.join(
            MODEL_DIR, "output", f"eval{use_eval}_test_set_preds.pkl"
        )
...
def build_training_data(
    ...
    chunk_dir = f"/tmp/insights_df_chunks/use_eval_{USE_EVAL}/"
    ...
    legit_first_df = _aggregate_first_insight(
        ...
        overwrite=overwrite_data or is_live,
    )
    ...
    if is_live:
        ...
        train_users, test_users = train_test_split(
            legit_users + cheat_users, test_size=0.1
        )
        ...
        save_pickle(
            {
                ...
                "eligible_player_dct": eligible_players,
            },
    ...
    if is_live:
        insights_df = pd.concat(
            [c for c in chunk_list if c is not None], ignore_index=True
        )
    else:
        paths = glob.glob(f"/tmp/insights_df_chunks/use_eval_{USE_EVAL}/*.pkl")
    ...
    for tc in insights_df["tc"].unique():
        for days in insights_df["days"].unique():
            subset = insights_df[
                (insights_df["tc"] == tc) & (insights_df["days"] == days)
            ].copy()
            ...
            data_entry: dict = {
                ...
                "valid_labels": va_labels,
                "test_labels": te_labels,
                "train_user_list": tr_users,
                "valid_user_list": va_users,
                "test_user_list": te_users,
