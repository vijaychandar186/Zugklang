...
class KaladinData:
    ...
    def __init__(
        self, days: int, tc: int, use_eval: int, data_dict: dict | None = None
    ) -> None:
        self.days = days
        self.tc = tc
        raw = (
        ...
        if use_eval:
            ...
            self.dense_train_input = self.dense_valid_input = self.dense_test_input = (
                None
            )
...
class KaladinModel(HyperModel):
    ...
    def build(self, hp) -> keras.Model:
        ...
        filter_modifiers = [
            hp.Int(f"fltr_mod_{d}", 1, 3, step=1) for d in self.data.conv_dimensions
        ]
        ...
        for i, _dim in enumerate(self.data.conv_dimensions):
    ...
    def _build_conv2d_branch(
        ...
        x = layers.Convolution2D(
            int(num_filters), kernel, strides=1, activation="relu"
        )(inp)
        for n in range(num_layers - 1):
            x = layers.Convolution2D(
                int(num_filters / (n + 2) + 1), 1, strides=1, activation="relu"
            )(x)
        if use_batch_norm:
