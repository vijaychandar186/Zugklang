...
(RUN_WS_E2E ? describe : describe.skip)('ws-server end-to-end', () => {
  ...
  let wsProcess: Bun.Subprocess<'ignore', 'ignore', 'ignore'> | undefined;
  let authPort = 0;
  let wsPort = 0;
  let wsBaseUrl = '';

  async function stopWsProcess(): Promise<void> {
    if (!wsProcess) return;
    try {
      wsProcess.kill('SIGKILL');
    } catch {
      void 0;
    }
    await Promise.race([wsProcess.exited, Bun.sleep(500)]);
    wsProcess = undefined;
  }
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      wsProcess = Bun.spawn([process.execPath, 'server.ts'], {
        cwd: SERVER_CWD,
        stdout: 'ignore',
        stderr: 'ignore',
        env: {
