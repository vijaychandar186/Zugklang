    ...
    (set, get) => ({
      ...
      handleEngineMessage: (message) => {
        ...
        if (message.startsWith('info')) {
          ...
          // Initialize pendingLines if needed, based on current state
          if (!pendingLines) {
          ...
          if (hasPv && hasMultiPv && hasScore) {
            try {
              ...
              // Update the specific line in our pending buffer
          ...
          // Ensure any pending updates are flushed before stopping
          if (pendingTimeout) {
