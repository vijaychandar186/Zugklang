...
Add to each service's `.env.production`:

```env
# Sentry — create a project at https://sentry.io
SENTRY_DSN=https://<key>@<org>.ingest.sentry.io/<project>
SENTRY_RELEASE=<git-sha>     # set by CI: $(git rev-parse --short HEAD)

# Frontend only — browser-side DSN (safe to be public)
NEXT_PUBLIC_SENTRY_DSN=https://<key>@<org>.ingest.sentry.io/<project>
# Source map upload during pnpm build (CI secret, not needed at runtime)
SENTRY_AUTH_TOKEN=<from Sentry → Settings → Auth Tokens>
SENTRY_ORG=<your-org-slug>
SENTRY_PROJECT=zugklang-frontend

# OpenTelemetry — already set in compose via OTEL_EXPORTER_OTLP_ENDPOINT
# Override here only if using an external OTel backend (e.g. Grafana Cloud)
OTEL_EXPORTER_OTLP_ENDPOINT=http://otel-collector:4318
```

**Sentry setup steps:**
1. Create a Sentry org at https://sentry.io/signup
2. Create three projects: `zugklang-frontend`, `zugklang-ws-server`, `zugklang-anti-cheat`
3. Copy the DSN from each project's Settings → Client Keys
4. Generate a `SENTRY_AUTH_TOKEN` from Settings → Auth Tokens (with `project:write` scope)
5. Add `SENTRY_AUTH_TOKEN` as a CI secret in GitHub Actions (Settings → Secrets → Actions)
