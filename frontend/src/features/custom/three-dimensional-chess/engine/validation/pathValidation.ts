...
export function isPathClear(
  ...
  debugLog(
    'pathValidation',
    `[isPathClear] Checking path from ${fromSquare.id} to ${toSquare.id}`
  );
  debugLog(
    'pathValidation',
    `[isPathClear] Path coordinates (${pathCoordinates.length}):`,
    pathCoordinates
  );
  ...
  for (const coord of pathCoordinates) {
    if (isCoordinateBlocked(coord, world, pieces)) {
      debugLog(
        'pathValidation',
        `[isPathClear] BLOCKED at coordinate (${coord.file}, ${coord.rank})`
      );
      return false;
    }
  }

  debugLog('pathValidation', `[isPathClear] Path is clear`);
  return true;
}
...
export function isDestinationBlockedByVerticalShadow(
  ...
  const squareIdsAtCoordinate = getSquareIdsForCoordinate(
    toSquare.file,
    toSquare.rank,
    world
  );
...
function isCoordinateBlocked(
  ...
  const squareIdsToCheck = getSquareIdsForCoordinate(
    coord.file,
    coord.rank,
    world
  );
...
export function isPieceAt(squareId: string, pieces: Piece[]): boolean {
  ...
  if (hasPiece) {
    debugLog(
      'pathValidation',
      `[isPieceAt] Found piece at ${squareId} (converted level: ${square.level})`
    );
  }
...
function instanceIdToBoardId(instanceId: string): string {
  ...
  debugLog(
    'pathValidation',
    `[instanceIdToBoardId] ${instanceId} → ${boardId}`
  );

  return boardId;
}

function parseSquareIdToPiece(
  squareId: string
): { file: number; rank: number; level: string } | null {
