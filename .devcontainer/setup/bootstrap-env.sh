...
frontend_redis_url="$(read_existing_value "$FRONTEND_ENV" "REDIS_URL" || printf '%s' "redis://localhost:6379")"
frontend_internal_api_secret="$(resolve_value "INTERNAL_API_SECRET" "$FRONTEND_ENV" "INTERNAL_API_SECRET" "change-me-internal-secret")"
...
anti_cheat_internal_api_secret="$(resolve_value "INTERNAL_API_SECRET" "$ANTI_CHEAT_ENV" "INTERNAL_API_SECRET" "change-me-internal-secret")"
anti_cheat_ban_threshold="$(read_existing_value "$ANTI_CHEAT_ENV" "BAN_THRESHOLD" || printf '%s' "0.9")"
...
if [[ "$NON_INTERACTIVE" -eq 0 ]]; then
  ...
  frontend_redis_url="$(prompt_value "REDIS_URL" "$frontend_redis_url")"
  frontend_internal_api_secret="$(prompt_value "INTERNAL_API_SECRET" "$frontend_internal_api_secret")"
  echo
  ...
  anti_cheat_internal_api_secret="$(prompt_value "INTERNAL_API_SECRET" "$anti_cheat_internal_api_secret")"
  anti_cheat_ban_threshold="$(prompt_value "BAN_THRESHOLD" "$anti_cheat_ban_threshold")"
  echo
else
...
REDIS_URL="$frontend_redis_url"
INTERNAL_API_SECRET="$frontend_internal_api_secret"
EOF
...
INTERNAL_API_SECRET=$anti_cheat_internal_api_secret
BAN_THRESHOLD=$anti_cheat_ban_threshold
EOF
