...
### Production Compose (toggle flags)

Use the wrapper script with `1/0` flags:

- `SENTRY_ENABLED` -> self-hosted Sentry stack (`docker-compose.sentry.yaml`)
- `MONITORING_ENABLED` -> exporters + Prometheus + Loki + Tempo + Promtail
- `GRAFANA_ENABLED` -> Grafana
- `OTEL_ENABLED` -> OpenTelemetry collector
- `DETACH` -> run in background (`1`) or foreground (`0`, default)

```bash
# Core app only
SENTRY_ENABLED=0 MONITORING_ENABLED=0 GRAFANA_ENABLED=0 OTEL_ENABLED=0 ./setup/compose-prod.sh up

# Core + Sentry
SENTRY_ENABLED=1 ./setup/compose-prod.sh up

# Background mode (detached)
DETACH=1 SENTRY_ENABLED=1 ./setup/compose-prod.sh up

# Core + monitoring, but without Grafana and OTel
MONITORING_ENABLED=1 GRAFANA_ENABLED=0 OTEL_ENABLED=0 ./setup/compose-prod.sh up

# Print selected services for current flags
./setup/compose-prod.sh services
```

Stop both (when enabled):

```bash
SENTRY_ENABLED=1 ./setup/compose-prod.sh down
```
