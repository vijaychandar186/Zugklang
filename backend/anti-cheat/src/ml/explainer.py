    ...
    class _InferenceWrapper:
        def predict(self, inputs, verbose: int = 0) -> np.ndarray:
            input_dict = {
                ...
                for key, inp in zip(input_keys, inputs, strict=False)
            }
...
def load_shap_explainer(model_directory: str) -> shap.DeepExplainer | None:
...
def compute_shap_explanations(
    ...
    df = pd.DataFrame(
        rows, columns=["user", "dimension", "insight", "shap_insight_sum"]
    )
    df["shap_dimension_sum"] = df.groupby(["user", "dimension"])[
        "shap_insight_sum"
    ].transform("sum")
...
def build_top_insight_urls(
    ...
    for i in range(1, top_n + 1):
        ...
        df[f"shap_score_{i}"] = np.where(
            df["rank"] == i, df["shap_insight_sum"], np.nan
        )
