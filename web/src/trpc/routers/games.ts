...
function toPgnResult(result: string, reason: string): '1-0' | '0-1' | '1/2-1/2' | '*' {
  if (reason === 'abort') return '*';
  const lower = result.toLowerCase();
  if (lower.includes('white wins') || lower.includes('white win') || lower.startsWith('1-0'))
    return '1-0';
  if (lower.includes('black wins') || lower.includes('black win') || lower.startsWith('0-1'))
    return '0-1';
...
export const gamesRouter = router({
  ...
  saveGame: privateProcedure.input(saveGameInputSchema).mutation(async ({ ctx, input }) => {
    ...
    try {
      ...
      if (
        ...
        return { duplicate: true, gameId: null, whiteRatingDelta: null, blackRatingDelta: null };
      }
      throw new TRPCError({ code: 'INTERNAL_SERVER_ERROR', message: 'Failed to save game' });
    }
  }),
  ...
  getMyGames: privateProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const where = { OR: [{ whiteUserId: userId }, { blackUserId: userId }] } as const;
