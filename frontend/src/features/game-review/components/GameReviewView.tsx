...
export function GameReviewView({
  ...
  const lastLiveBoardUpdateRef = useRef(0);
  const {
    pgn,
    depth,
    status,
  ...
  const handleReview = useCallback(async () => {
    ...
    try {
      ...
      setLiveEvaluations(Array.from({ length: positions.length }, () => null));
      setProgress(10);
      lastLiveBoardUpdateRef.current = 0;
      setStatus('evaluating');
      const { evaluatePositions } = await import('@/lib/engine/evaluation');
      positions = await evaluatePositions(
        positions,
        depth,
        (current, total, percent, latestPosition) => {
          const evalProgress = 10 + Math.round(percent * 0.8);
          setProgress(evalProgress);
          const topLine = latestPosition.topLines?.[0];
          if (topLine?.evaluation) {
            updateLiveEvaluation(current - 1, {
          ...
          const shouldUpdateBoard =
            current === total || now - lastLiveBoardUpdateRef.current >= 120;
          if (shouldUpdateBoard) {
            goToMove(current);
            lastLiveBoardUpdateRef.current = now;
          }
        }
      );
      setStatus('reporting');
      ...
      {(report || (status === 'evaluating' && liveEvaluations.length > 0)) && (
        <div className='shrink-0'>
          <EvaluationGraph
            ...
            liveEvaluations={report ? undefined : liveEvaluations}
