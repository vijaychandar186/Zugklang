import {
  Piece,
  AttackBoardStates
} from '@/features/custom/three-dimensional-chess/store/gameStore';
import { ChessWorld, WorldSquare } from '../world/types';
import { createSquareId } from '../world/coordinates';
import { getLegalMoves } from './moveValidator';
import { resolveBoardId } from '@/features/custom/three-dimensional-chess/utils/resolveBoardId';
import { TrackStates } from './types';

export function isSquareAttacked(
  square: WorldSquare,
  byColor: 'white' | 'black',
  world: ChessWorld,
  pieces: Piece[],
  attackBoardStates?: AttackBoardStates,
  trackStates?: TrackStates
): boolean {
  const attackingPieces = pieces.filter((p) => p.color === byColor);

  for (const piece of attackingPieces) {
    const legalMoves = getLegalMoves(
      piece,
      world,
      pieces,
      attackBoardStates,
      trackStates
    );
    if (legalMoves.includes(square.id)) {
      return true;
    }
  }

  return false;
}

export function isInCheck(
  color: 'white' | 'black',
  world: ChessWorld,
  pieces: Piece[],
  attackBoardStates?: AttackBoardStates,
  trackStates?: TrackStates
): boolean {
  const king = pieces.find((p) => p.type === 'king' && p.color === color);

  if (!king) {
    return false;
  }

  const resolvedLevel = resolveBoardId(king.level, attackBoardStates);
  const kingSquareId = createSquareId(king.file, king.rank, resolvedLevel);
  const kingSquare = world.squares.get(kingSquareId);

  if (!kingSquare) {
    return false;
  }

  const opponentColor = color === 'white' ? 'black' : 'white';
  return isSquareAttacked(
    kingSquare,
    opponentColor,
    world,
    pieces,
    attackBoardStates,
    trackStates
  );
}

export function getLegalMovesAvoidingCheck(
  piece: Piece,
  world: ChessWorld,
  pieces: Piece[],
  attackBoardStates?: AttackBoardStates,
  trackStates?: TrackStates
): string[] {
  const allMoves = getLegalMoves(
    piece,
    world,
    pieces,
    attackBoardStates,
    trackStates
  );
  const safeMoves: string[] = [];

  for (const toSquareId of allMoves) {
    const { newPieces } = simulateMove(pieces, piece, toSquareId, world);

    if (
      !isInCheck(piece.color, world, newPieces, attackBoardStates, trackStates)
    ) {
      safeMoves.push(toSquareId);
    }
  }

  return safeMoves;
}

function simulateMove(
  pieces: Piece[],
  piece: Piece,
  toSquareId: string,
  world: ChessWorld
): { newPieces: Piece[]; capturedPiece: Piece | null } {
  const toSquare = world.squares.get(toSquareId);
  if (!toSquare) {
    return { newPieces: pieces, capturedPiece: null };
  }

  const toLevelMatch = toSquareId.match(/^[a-z]\d+(.+)$/);
  const toLevel = toLevelMatch ? toLevelMatch[1] : piece.level;

  // Match captures by comparing resolved instance IDs so we handle board vs instance mismatch
  const capturedPiece =
    pieces.find((p) => {
      const resolvedLevel = resolveBoardId(p.level);
      return (
        p.file === toSquare.file &&
        p.rank === toSquare.rank &&
        resolvedLevel === toLevel &&
        p.id !== piece.id
      );
    }) || null;

  const newPieces = pieces
    .filter((p) => p.id !== capturedPiece?.id)
    .map((p) => {
      if (p.id === piece.id) {
        return {
          ...p,
          file: toSquare.file,
          rank: toSquare.rank,
          level: toLevel,
          hasMoved: true
        };
      }
      return p;
    });

  return { newPieces, capturedPiece };
}

export function isCheckmate(
  color: 'white' | 'black',
  world: ChessWorld,
  pieces: Piece[],
  attackBoardStates?: AttackBoardStates,
  trackStates?: TrackStates
): boolean {
  if (!isInCheck(color, world, pieces, attackBoardStates, trackStates)) {
    return false;
  }

  const playerPieces = pieces.filter((p) => p.color === color);

  for (const piece of playerPieces) {
    const safeMoves = getLegalMovesAvoidingCheck(
      piece,
      world,
      pieces,
      attackBoardStates,
      trackStates
    );
    if (safeMoves.length > 0) {
      return false;
    }
  }

  return true;
}

export function isStalemate(
  color: 'white' | 'black',
  world: ChessWorld,
  pieces: Piece[],
  attackBoardStates?: AttackBoardStates,
  trackStates?: TrackStates
): boolean {
  if (isInCheck(color, world, pieces, attackBoardStates, trackStates)) {
    return false;
  }

  const playerPieces = pieces.filter((p) => p.color === color);

  for (const piece of playerPieces) {
    const safeMoves = getLegalMovesAvoidingCheck(
      piece,
      world,
      pieces,
      attackBoardStates,
      trackStates
    );
    if (safeMoves.length > 0) {
      return false;
    }
  }

  return true;
}
