...
export class Chess {
  ...
  isThreefoldRepetition(): boolean {
    const currentFen = makeFen(this._pos.toSetup()).split(' ').slice(0, 4).join(' ');
    let count = 1;
    for (const entry of this._historyStack) {
      const histFen = makeFen(entry.position.toSetup()).split(' ').slice(0, 4).join(' ');
      if (histFen === currentFen) count++;
