...
type CheckersChessSetupDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function CheckersChessSetupDialog({
  open,
  onOpenChange
}: CheckersChessSetupDialogProps) {
  const autoFlipBoard = useChessStore((s) => s.autoFlipBoard);
  const setAutoFlipBoard = useChessStore((s) => s.setAutoFlipBoard);
  const startNewGame = useCheckersChessStore((s) => s.startNewGame);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-[500px]'>
        <DialogHeader>
          <DialogTitle>Chess with Checkers</DialogTitle>
        </DialogHeader>
        ...
        <div className='space-y-6 py-4'>
          ...
          <div className='rounded-lg border p-4'>
            <p className='text-muted-foreground text-center text-sm'>
              Standard chess rules — all pieces just look like checkers!
              <br />
