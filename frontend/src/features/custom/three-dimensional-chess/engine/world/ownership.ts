...
export function getVacantPins(
  fromPinId: string,
  attackBoardPositions: Record<string, string>
): string[] {
  return getAdjacentPins(fromPinId).filter(
    (p) => !isPinOccupied(p, attackBoardPositions)
  );
}

export function getPinOwner(
  pinId: string,
  attackBoardPositions: Record<string, string>
): 'white' | 'black' | null {
...
export function isSidewaysMove(
  fromPinId: string,
  toPinId: string,
  boardId: string
): boolean {
...
export function deriveInstanceIdForBoard(
  boardId: string,
  pinId: string,
  rotation: number
): string {
...
export function updateInstanceRotation(
  instanceId: string,
  rotation: 0 | 180
): string {
