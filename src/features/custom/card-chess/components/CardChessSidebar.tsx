...
export function CardChessSidebar({ mode }: CardChessSidebarProps) {
  ...
  // Read game state from card store
  ...
  return (
    <>
      <div className='bg-card flex min-h-[300px] flex-col rounded-lg border lg:h-full'>
        ...
        {/* Card Panel in sidebar when game is active */}
        {gameStarted && !gameOver && <CardPanel turnColor={turn} />}
      ...
      <CardChessSetupDialog open={newGameOpen} onOpenChange={setNewGameOpen} />
    </>
  );
}
