...
def _material_imbalance(board: chess.Board, is_white: bool) -> int:
    ...
    mine = sum(_PIECE_VALUES[pt] * len(board.pieces(pt, my_color)) for pt in _PIECE_VALUES)
    opp = sum(_PIECE_VALUES[pt] * len(board.pieces(pt, opp_color)) for pt in _PIECE_VALUES)
    return mine - opp
...
def _movetime_variance_score(times_ms: List[int]) -> Optional[int]:
...
def ingest_game(data: GameIngestionRequest, db: Prisma) -> Tuple[int, int]:
    ...
    def _build_moves(move_data: List[Dict], times_ms: List[int]) -> List[Dict]:
        ...
        rows = []
        for md, t_ms in zip(move_data, times_ms):
            row: Dict = {
    ...
    if data.white_user_id and white_move_data and data.move_times_white_ms:
        ...
        insight = db.gameinsight.create(data={
