...
export function updateInstanceVisibility(
  world: ChessWorld,
  trackStates: TrackStates
) {
  debugLog('visibility', '[updateInstanceVisibility] START', { trackStates });
  ...
  debugLog(
    'visibility',
    '[updateInstanceVisibility] Should show:',
    instancesToShow
  );
  ...
  const show = (track: 'QL' | 'KL', pin: number, rot: 0 | 180) => {
    ...
    if (board) {
      debugLog('visibility', '[updateInstanceVisibility] Showing:', id);
  ...
  debugLog('visibility', '[updateInstanceVisibility] END');
}
...
export function setVisibleInstances(world: ChessWorld, instanceIds: string[]) {
  debugLog('visibility', '[setVisibleInstances] START', { instanceIds });
  ...
  instanceIds.forEach((instanceId) => {
    ...
    if (board) {
      debugLog('visibility', '[setVisibleInstances] Showing:', instanceId);
      board.isVisible = true;
      board.isAccessible = true;
  ...
  debugLog(
    'visibility',
    '[setVisibleInstances] Shown',
    shownInstances.length,
    'instances:',
    shownInstances
  );

  debugLog('visibility', '[setVisibleInstances] END');
}
