...
describe('game handler integration', () => {
  ...
  test('abort and game_over_notify mark room ended', () => {
    ...
    room.moves = ['e2e4']; // guard requires at least one server-recorded move
  ...
  test('accept rematch creates a new room with colors swapped', async () => {
    const { white, black, room } = setupRoom(
    ...
    await handleAcceptRematch(asBunWs(black));
