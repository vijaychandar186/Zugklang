...
import {
  isPathClear,
  isPieceAt,
  isDestinationBlockedByVerticalShadow,
  getPathCoordinates
} from './pathValidation';
...
import { resolveBoardId } from '@/features/custom/three-dimensional-chess/utils/resolveBoardId';
...
export function validatePawnMove(context: MoveValidationContext): MoveResult {
  ...
  if (fromLevel !== toLevel) {
    ...
    if (isFromAttackBoard && isToAttackBoard) {
      ...
      if (fromPinMatch && toPinMatch) {
        ...
        if (
          fromPin !== toPin &&
          (fromSquare.file === 0 || fromSquare.file === 5)
        ) {
          return {
            valid: false,
            reason:
              'z and e files have no main board connectivity between attack boards'
          };
        }
      }
    }
  ...
  // Need to resolve piece levels (board IDs like "BQL") to instance IDs (like "QL5:0") for comparison
  const enemyAtDestination = allPieces.some((p) => {
  ...
  if (
    ...
    if (trackStates) {
      ...
      if (promotionCheck.reason === 'E_NONEXISTENT_TARGET') {
        return {
          valid: false,
          reason: 'Cannot move: promotion square does not exist'
        };
      }
...
export function validateRookMove(context: MoveValidationContext): MoveResult {
  ...
  if (levelChange) {
    ...
    if (isFromAttackBoard && isToAttackBoard) {
      ...
      debugLog(
        'moveValidation',
        `[validateRookMove] Attack board to attack board: ${fromSquare.id} -> ${toSquare.id}`
      );
      debugLog(
        'moveValidation',
        `[validateRookMove] fromTrack=${fromTrack}, toTrack=${toTrack}`
      );

      if (fromTrack !== toTrack) {
        debugLog(
          'moveValidation',
          `[validateRookMove] BLOCKED: Different tracks`
        );
        return { valid: false, reason: 'cannot move between QL and KL tracks' };
      }
      ...
      if (fromPinMatch && toPinMatch) {
        ...
        debugLog(
          'moveValidation',
          `[validateRookMove] Pin check: fromPin=${fromPin}, toPin=${toPin}`
        );
        ...
        if (fromPin !== toPin) {
          ...
          if (fromSquare.file === 0 || fromSquare.file === 5) {
            debugLog(
              'moveValidation',
              `[validateRookMove] BLOCKED: Cannot move along z/e file between different pins (${fromPin} -> ${toPin})`
            );
            return {
              valid: false,
              reason:
          ...
          debugLog(
            'moveValidation',
            `[validateRookMove] Different pins detected. Path coordinates: ${pathCoords.length}`
          );

          if (pathCoords.length === 0) {
            debugLog(
              'moveValidation',
              `[validateRookMove] BLOCKED: Different pins (${fromPin} -> ${toPin}) with no intermediate path`
            );
            debugLog(
              'moveValidation',
              `[validateRookMove] Details: ${fromSquare.id} (pin ${fromPin}) -> ${toSquare.id} (pin ${toPin})`
            );
            return {
              valid: false,
              reason: 'attack boards at different pins not adjacent'
            };
          }

          debugLog(
            'moveValidation',
            `[validateRookMove] Different pins but has intermediate path, will check if path is clear`
          );
        }
      }
      ...
      debugLog(
        'moveValidation',
        `[validateRookMove] ALLOWED: Same track connection`
      );
    }
  }
  ...
  if (!movesOnOneAxis) {
    debugLog(
      'moveValidation',
      `[validateRookMove] BLOCKED: Not moving on one axis (fileChange=${fileChange}, rankChange=${rankChange})`
    );
    return { valid: false, reason: 'rook must move in straight line' };
  }

  if (!isPathClear(fromSquare, toSquare, world, allPieces)) {
    debugLog('moveValidation', `[validateRookMove] BLOCKED: Path not clear`);
  ...
  if (
    !pieceAtDestination &&
    isDestinationBlockedByVerticalShadow(toSquare, world, allPieces)
  ) {
...
export function validateBishopMove(context: MoveValidationContext): MoveResult {
  ...
  if (!validDiagonal) {
    if (toLevel.startsWith('QL6')) {
      debugLog(
        'moveValidation',
        `[validateBishopMove] QL6 move REJECTED: not a valid diagonal. fileChange=${fileChange}, rankChange=${rankChange}, levelDiff=${levelDiff}`
      );
    }
    return {
      valid: false,
      reason:
        'bishop must move diagonally (same distance in exactly 2 dimensions)'
    };
  }
...
export function validateKingMove(context: MoveValidationContext): MoveResult {
  ...
  // Check if moving between different attack board instances
  // Kings CAN move between attack boards and main boards (W, N, B)
  // but NOT between disconnected attack boards (different tracks or pins)
  if (levelChange) {
    const isFromAttackBoard = fromLevel.includes(':');
    const isToAttackBoard = toLevel.includes(':');
  ...
  // Check if destination square has a piece on it (for capture)
  const pieceAtDestination = allPieces.some((p) => {
    const resolvedPieceLevel = resolveBoardId(p.level, attackBoardStates);
    return (
      p.file === toSquare.file &&
  ...
  // Per official Meder rules Article 3.1(c), vertical shadow blocking applies to all pieces
  // except knights. Kings are NOT exempt - only knights can jump over vertical shadows.
  // Vertical shadow rule only applies to EMPTY destination squares.
  // If there's a piece at the destination (capture), vertical shadow doesn't matter.
  if (
    !pieceAtDestination &&
