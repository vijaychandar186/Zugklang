...
/** Get or create a Rating record for a user+variant */
...
export async function POST(req: NextRequest) {
  ...
  if (isRated) {
    ...
    const [whiteRating, blackRating] = await Promise.all([
      getOrCreateRating(whiteUserId!, variant),
      getOrCreateRating(blackUserId!, variant)
    ]);
    ...
    await prisma.$transaction([
      prisma.rating.update({
        where: { userId_variant: { userId: whiteUserId!, variant } },
        data: {
      ...
      prisma.rating.update({
        where: { userId_variant: { userId: blackUserId!, variant } },
        data: {
