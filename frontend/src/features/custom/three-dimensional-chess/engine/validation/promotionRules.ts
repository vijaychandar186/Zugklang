/**
 * Pawn Promotion Logic for 3-Dimensional Chess
 *
...
export function promotionSquareExists(
  ...
  const boardId = determineBoardId(
    file,
    rank,
    color,
    trackStates,
...
export function determineBoardId(
  ...
  if (fileStr === 'z' || fileStr === 'a') {
    ...
    const pin =
      boardOwner === 'white'
        ? trackStates.QL.whiteBoardPin
        : trackStates.QL.blackBoardPin;
    const rotation =
      boardOwner === 'white'
        ? trackStates.QL.whiteRotation
        : trackStates.QL.blackRotation;
  ...
  if (fileStr === 'd' || fileStr === 'e') {
    ...
    const pin =
      boardOwner === 'white'
        ? trackStates.KL.whiteBoardPin
        : trackStates.KL.blackBoardPin;
    const rotation =
      boardOwner === 'white'
        ? trackStates.KL.whiteRotation
        : trackStates.KL.blackRotation;
...
export function checkCornerOverhang(
  ...
  const opponentKey =
    opponentColor === 'white' ? 'whiteBoardPin' : 'blackBoardPin';
  const opponentPin = trackStates[track][opponentKey];
