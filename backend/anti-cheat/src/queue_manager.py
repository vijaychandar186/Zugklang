    ...
    def start(self) -> None:
        while True:
            ...
            while len(pending) < BATCH_SIZE:
                ...
                if (
                    pending
                    and (datetime.utcnow() - batch_start).total_seconds()
                    > BATCH_TIMEOUT
                ):
                    break
            if not pending:
    ...
    def _write_results(self, completed: list[Record]) -> None:
        ...
        for doc in completed:
            ...
            if r.get("error") is None:
                ...
                pred = r.get("pred")
                if user_id and pred is not None and pred >= BAN_THRESHOLD:
                    redis.set(f"banned:{user_id}", "1")
                    log.warning(
                        "Auto-banned user %s userId=%s (pred=%.4f >= threshold=%.2f)",
                        doc["username"],
                        user_id,
                        pred,
                        BAN_THRESHOLD,
