...
type ChessStore = {
  ...
  // Callback for new game (used by board component)
  onNewGame: () => void;
...
export const useChessStore = create<ChessStore>()(
  persist(
    (set, get) => ({
      ...
      setMode: (mode) => {
        if (mode === 'analysis') {
          // When switching to analysis, reset game state but keep position
          set({
            mode,
            gameOver: false,
            gameStarted: false,
