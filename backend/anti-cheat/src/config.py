...
# Prediction score at which a user is automatically banned from multiplayer.
BAN_THRESHOLD: float = float(config.get("BAN_THRESHOLD", "0.9"))
...
def resolve_model_dir_path(use_eval: int, tc: int, days: int) -> str:
    ...
    if os.path.exists(preferred_saved_model) or os.path.exists(
        preferred_legacy_saved_model
    ):
        return preferred
