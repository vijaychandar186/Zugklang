...
import { DICE_SOUND_PATH } from '@/lib/public-paths';
import {
  SignalIndicator,
  AbandonCountdown
...
import { type DicePiece, type DiceResult } from '../stores/useDiceChessStore';
...
function checkPieceHasValidMoves(game: Chess, piece: DicePiece): boolean {
  const allMoves = game.moves({ verbose: true }) as {
    piece: string;
  }[];
  return allMoves.some((m) => m.piece === piece);
}
export function getHighlightsForDice(
  game: Chess,
  dice: DiceResult[]
...
export interface DiceChessMultiplayerGame {
  currentFEN: string;
  gameStarted: boolean;
  gameOver: boolean;
...
export function useDiceChessMultiplayerGame(
  challengeId?: string
): DiceChessMultiplayerGame {
