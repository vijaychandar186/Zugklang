...
export async function handleCreateChallenge(
  ...
  if (ws.data.userId) {
    const banned = await redis.get(`banned:${ws.data.userId}`);
    if (banned) {
      send(ws, {
        type: 'error',
  ...
  if (ws.data.challengeId) {
    const oldId = ws.data.challengeId;
    challenges.delete(oldId);
    void redis.del(`challenge:${oldId}`);
  ...
  // Send confirmation immediately (before Redis persistence)
  ...
  void redis
    ...
    .then(() => redis.expire(`challenge:${challengeId}`, CHALLENGE_TTL_SEC))
    .catch(() => {
      /* non-fatal: in-memory store is authoritative for same-pod joins */
    });
}
...
export async function handleJoinChallenge(
  ...
  // Check in-memory first (same pod — no Redis round-trip needed)
  const localChallenge = challenges.get(challengeId);
  if (localChallenge) {
    if (
      localChallenge.creator.data.id === ws.data.id ||
      (localChallenge.creatorUserId &&
        localChallenge.creatorUserId === ws.data.userId)
    ) {
    ...
    delete localChallenge.creator.data.challengeId;
    void redis.del(`challenge:${challengeId}`);

    let white: BunWS, black: BunWS;
    if (localChallenge.creatorColor === 'white') {
      [white, black] = [localChallenge.creator, ws];
    } else if (localChallenge.creatorColor === 'black') {
      [white, black] = [ws, localChallenge.creator];
    } else {
      [white, black] =
        Math.random() < 0.5
          ? [localChallenge.creator, ws]
          : [ws, localChallenge.creator];
    }
    await createRoom(white, black, localChallenge.variant, localChallenge.timeControl);
  ...
  // Fall through to Redis path (cross-pod challenge)
...
export async function handleCancelChallenge(ws: BunWS): Promise<void> {
  if (ws.data.challengeId) {
    const cid = ws.data.challengeId;
    challenges.delete(cid);
    delete ws.data.challengeId;
    void redis.del(`challenge:${cid}`);
  }
}
