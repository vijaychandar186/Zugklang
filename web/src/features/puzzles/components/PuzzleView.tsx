...
export function PuzzleView({ initialBoard3dEnabled }: PuzzleViewProps = {}) {
  ...
  const [lastPuzzleRatingDelta, setLastPuzzleRatingDelta] = useState<number | null>(null);
  const {
    currentPuzzle,
    puzzleIndex,
    difficulty,
  ...
  useEffect(() => {
    ...
    fetch('/api/training/puzzle-attempt', {
      ...
      body: JSON.stringify({
        puzzleId: currentPuzzle.FEN.split(' ')[0],
        difficulty,
  ...
  return (
    <GameViewLayout
      ...
      bottomPlayerInfo={
        <PlayerInfo
          name={session?.user?.name ?? (playerColor === 'white' ? 'White' : 'Black')}
          subtitle='You'
          image={session?.user?.image}
          isStockfish={false}
          rating={myPuzzleRating}
        />
      }
      ...
      sidebar={
        <>
          <SidebarPanel className='p-4'>
            ...
            {lastPuzzleRatingDelta != null && (
              <p
                className={`mt-2 text-center font-mono text-xs font-semibold ${lastPuzzleRatingDelta >= 0 ? 'text-green-500' : 'text-red-500'}`}
              >
                {lastPuzzleRatingDelta >= 0 ? '+' : ''}
                {lastPuzzleRatingDelta} puzzle rating
              </p>
            )}
          </SidebarPanel>
