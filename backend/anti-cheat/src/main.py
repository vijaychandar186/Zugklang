...
def _init_observability() -> None:
    """Initialise OTel distributed tracing. Skipped if no endpoint is configured."""
...
_preload_explainers = (
    os.environ.get("ANTI_CHEAT_PRELOAD_EXPLAINERS", "false").lower() == "true"
)
...
async def lifespan(app: FastAPI):
    ...
    if _preload_explainers:
        ...
        log.info(
            "Skipping explainer preload at startup (ANTI_CHEAT_PRELOAD_EXPLAINERS=false)."
        )

    # Start the analysis queue worker in a background thread.
    # Wraps QueueManager.start() so the thread restarts on unexpected crashes.
    ...
    t.start()
    log.info("Analysis queue worker started.")
...
@app.post(
    "/game",
    response_model=GameIngestionResponse,
    tags=["Ingestion"],
    dependencies=[Security(_require_internal_auth)],
)
async def ingest_game_endpoint(data: GameIngestionRequest) -> GameIngestionResponse:
    ...
    try:
        ...
        # Enqueue authenticated players for ML analysis.
        players_to_queue = []
        if data.white_username and white_n > 0:
            players_to_queue.append((data.white_username, data.white_user_id))
        if data.black_username and black_n > 0:
            players_to_queue.append((data.black_username, data.black_user_id))

        for username, user_id in players_to_queue:
            _db.analysisqueue.upsert(
                where={"username": username},
                data={
                    "create": {"username": username, "userId": user_id, "priority": 0},
                    "update": {"respondedAt": None, "startedAt": None, "error": None},
                },
            )
            r.rpush("analysis:queue:notify", json.dumps({"username": username}))
            log.info("Queued %s for analysis.", username)
...
@app.post(
    "/queue/start", tags=["Queue"], dependencies=[Security(_require_internal_auth)]
)
...
@app.post("/train", tags=["Model"], dependencies=[Security(_require_internal_auth)])
